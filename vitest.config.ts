...
export default defineConfig({
  ...
  test: {
    ...
    // Subprocess-backed suites (hooks, subprocess, shell, terminal) spawn real
    // child processes and poll for effects; under the aggregate fork load of a
    // full-suite run they exceed Vitest's 5s default. Give them room to finish
    // under contention while a genuinely hung test still fails within a bounded
    // window (the snapshot suite already uses a larger timeout for the same
    // reason).
    testTimeout: 30_000,
    ...
    projects: [
      {
        ...
        test: {
          ...
          // Subprocess-backed suites (hooks, subprocess, shell, terminal) spawn
          // real children and poll for effects; under full-suite fork load they
          // exceed the 5s default. Give them room while hung tests still fail
