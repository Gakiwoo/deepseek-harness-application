...
describe('desktop tray', () => {
  it('shows the approved menu and routes native actions', () => {
    ...
    createDesktopTray(fake.native, 'linux', show, requestQuit)
  ...
  it('uses a macOS template image', () => {
    const fake = fakeNative()
    createDesktopTray(fake.native, 'darwin', vi.fn(), vi.fn())
  ...
  it('removes listeners and destroys the tray once', () => {
    const fake = fakeNative()
    const tray = createDesktopTray(fake.native, 'linux', vi.fn(), vi.fn())
    tray.dispose()
    tray.dispose()
  ...
  it('fails loud when the native tray image is empty', () => {
    ...
    expect(() => createDesktopTray(fake.native, 'linux', vi.fn(), vi.fn())).toThrow(
