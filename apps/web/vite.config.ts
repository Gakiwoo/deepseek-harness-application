...
export default defineConfig(({ mode }) => ({
  // Desktop builds load from the dsh:// custom protocol (opaque origin), so
  // assets must resolve relative to the document; the web deploy serves from
  // the site root. Only the base differs between the two modes.
  base: mode === 'desktop' ? './' : '/',
