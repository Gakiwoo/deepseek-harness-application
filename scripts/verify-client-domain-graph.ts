...
function checkPackage(pkgName: string, clientDir: string): Violation[] {
  ...
  for (const rel of files) {
    ...
    for (const match of source.matchAll(/from\s+['"](\.[^'"]+)['"]/g)) {
      ...
      // A relative import that climbs past the client root (parts underflow)
      // is a package-level edge to src/<pkg>/..., not a client domain edge —
      // package-level rules (verify-module-graph) govern it.
      if (escaped) continue
