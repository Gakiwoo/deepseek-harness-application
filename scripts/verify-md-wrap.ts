...
/**
 * Working directories whose step-based documents (checklists, Run:/Expected:
 * steps) legitimately hard-wrap and are not subject to the one-line-per-paragraph
 * prose rule. Planning artifacts under superpowers are excluded; shipped prose
 * documentation stays checked.
 */
...
const files = uniqueRepoFiles(root, PATTERNS, isArchivedAgentNotePath)
  .filter(file => !EXCLUDED_DIRS.some(dir => relative(root, file.abs).startsWith(`${dir}/`)))
