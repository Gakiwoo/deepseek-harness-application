...
/**
 * App packages under release-member directories that are never published to npm:
 * they ship as distributable artifacts (electron-builder, CI) and stay private.
 */
const privateAppPackages = new Set(['@deepseek-ai/dsh-desktop'])
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  ...
  // The desktop shell is built by electron-builder, not published to npm; the
  // files field lists what electron-builder packs into the asar archive.
  '@deepseek-ai/dsh-desktop': ['out/main.js', 'out/preload.cjs', 'package.json'],
}
...
function expectedDshPackageFiles(manifest: PackageManifest): readonly string[] {
  ...
  return [
    ...
    // The Electron preload and main process share this browser-safe wire
    // module through a standalone ESM subpath bundle.
    ...hasExportPair(manifest, './desktop-bridge', './lib/types/client/desktop-bridge.d.ts', './lib/desktop-bridge.js')
    ...
    ...hasExportPair(manifest, './client/typert', './lib/typert.client.d.ts', './lib/typert.client.js')
      ...
      : [],
    // A surface bundle's Electron-free host boot ships as its own bundle: the
    // desktop shell imports it from the packaged host closure beside the entry.
    ...hasExportPair(manifest, './host-boot', './lib/types/host-boot.d.ts', './lib/host-boot.js')
      ? ['lib/host-boot.js']
      : [],
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    // Private apps ship as distributable artifacts rather than npm packages
    // (electron-builder, CI); they stay private and carry no publish metadata.
    if (manifest.name && privateAppPackages.has(manifest.name)) {
      if (manifest.private !== true) {
