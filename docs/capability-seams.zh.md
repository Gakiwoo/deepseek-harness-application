...
flowchart LR
  ...
  svc_desktopRuntime["ctx.desktopRuntime<br/>Desktop surface runtime face"]
...
| `ctx.desktopRuntime` | `core` | [`desktop-app`](../packages/bundle/desktop-app) | - | - | - | Electron 壳之于已定居桌面树的消费面：/api 分发、启动图、client bundle 路径与前端 index 路径；壳是应用而非图内包。 |
