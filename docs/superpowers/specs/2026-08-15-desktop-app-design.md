...
- [ ] **Step 3.1: 写失败测试**（注入式 fetcher 的一元调用 + rpcId 校验 + 非法 target）
...
- [ ] **Step 4.1: 在 connection.client.spec.ts 追加失败测试**
...
- [ ] **Step 5.5: 修改 src/index.ts 的 apply()**
...
- [ ] **Step 6.3: 修改 ClientModuleRegistry**
...
- [ ] **Step 7.1:** Read `packages/boot/app-boot/tests/` 目录，选定覆盖 `PROFILE_TEMPLATES`/`loadProfile` 的既有 spec 文件。
...
- [ ] **Step 8.1: package.json**（以 `packages/bundle/web-app/package.json` 为底，按下列差异修改——diff 列表即全部改动）：
...
- [ ] **Step 8.2: cordis.patch.yml**（以 web-app 的 patch 为底；保留区域逐字复制，差异如下）
...
- [ ] **Step 9.3: 实现 host-boot.ts**（对照 `apps/cli/src/profile-boot.ts` 逐段裁剪；无 CLI flags/telemetry/信号处理——Electron main 拥有生命周期）
...
- [ ] **Step 10.3: fetch-pump.ts（先写测试再实现）**
...
- [ ] **Step 10.5: window.ts + splash.html**
...
- [ ] **Step 10.7: electron-builder.yml**
...
- [ ] **Step 11.2: scripts/pack-desktop.ts**
...
- [ ] **Step 11.3: 根 package.json scripts**
...
- [ ] **Step 14.1:** 按上述清单写全部文档（中英双语齐全，README.i18n.yaml 键对照 web-app）。
- [ ] **Step 14.2: 全量 gates**
