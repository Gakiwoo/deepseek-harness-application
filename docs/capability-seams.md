...
| `ctx.desktopRuntime` | `core` | [`desktop-app`](../packages/bundle/desktop-app) | - | - | - | The Electron shell consumption face over the settled desktop tree: /api dispatch, the boot graph, client bundle paths, and the frontend index path; the shell is an app, not a graph package. |
