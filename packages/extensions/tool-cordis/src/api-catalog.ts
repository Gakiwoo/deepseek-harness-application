...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'desktopRuntime',
    summary: 'The Electron main process\'s consumption face over the settled desktop tree.',
    description: 'The Electron main process\'s consumption face over the settled desktop tree.',
    methods: [
      {
        signature: 'fetch(request: Request): Promise<Response>',
        description: '/api dispatch (TypertGateway interceptor + apiproxy fallback) — the IPC pump\'s carrier.',
        parameters: [{ name: 'request', description: 'the fetch request to dispatch against the /api face.' }],
        returns: 'the response from the host dispatch.',
      },
      {
        signature: 'graph(): WebBootGraph',
        description: 'Current composed `window.__DSH_BOOT__` graph.',
        parameters: [],
        returns: 'the composed client module graph.',
      },
      {
        signature: 'clientPath(id: string): string | undefined',
        description: 'Absolute path of one plugin\'s built client bundle.',
        parameters: [{ name: 'id', description: 'the client module package id.' }],
        returns: 'the resolved bundle path, or undefined when the id is not in the graph.',
      },
      {
        signature: 'frontendIndex(): string',
        description: 'Absolute path of the frontend index.html this surface serves.',
        parameters: [],
