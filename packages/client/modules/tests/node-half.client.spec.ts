...
describe('client module graph without webServer (desktop surface)', () => {
  /** Construct the node-half service with a loader but no webServer service. */
  ...
  it('composes the graph and resolves client paths without webServer', () => {
    const packageName = '@fixture/desktop-ui'
    const clientPath = writePackage(packageName)
    mkdirSync(dirname(clientPath), { recursive: true })
    writeFileSync(clientPath, 'module.exports = {}\n')
