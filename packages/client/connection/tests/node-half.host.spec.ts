...
describe('connection node half without webServer (desktop surface)', () => {
  /** Minimal ApiProxy stub: session.list answers ok; everything else 404s the carrier. */
  function minimalApiProxy(): ApiProxy {
    return {
      sessions: { list: async () => ({ rpcId: RpcId('t1'), result: { ok: true, value: [] } }) },
  ...
  it('provides ctx.connection and serves /api through handle.fetch with no webServer', async () => {
    const { connection, dispose } = await mounted()
    const response = await connection.fetch(new Request('http://127.0.0.1/api/session.list', {
      method: 'POST',
  ...
  it('rejects a dedicated channel registration without webServer (desktop v1 carries /api only)', async () => {
    const { connection, dispose } = await mounted()
    expect(() => connection.rpc.handle('/rpc', async () => ({ ok: true, value: null }), {
      authority: 'loopback',
    })).toThrow(/requires the webServer surface/)
    await dispose()
  })
})
