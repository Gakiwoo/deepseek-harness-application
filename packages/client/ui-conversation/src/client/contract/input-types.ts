/**
 * Shared input-machine contract. Types
 * only. Moves the input types that cross domain boundaries (composer blocks,
 * the public action face, the input-state currency, session-addressed access)
 * into the contract layer so sibling domains import them here instead of the
 * input implementation. The `input/contract.ts` file re-exports them for its
 * package-internal consumers.
 */
import type { ClientContext, SessionId, SnapshotStore } from '@deepseek-ai/dsh-client-runtime/client'
import type { Branded } from '@deepseek-ai/dsh-brand'
import type {
  ArbitrateKey, ArbitrateOutcome, CommandClaim, ReferenceInsert, TokenSpan,
