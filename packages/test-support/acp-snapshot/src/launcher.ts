  ...
  const child = spawn(
    ...
    launch.args,
    {
      ...
      env: {
        ...process.env,
        ...launch.env,
        // The shipped apps persist sessions through Node's built-in `node:sqlite`,
        // which emits an ExperimentalWarning on stderr. Snapshot suites assert a
        // clean stderr, so suppress the warning for every ACP child (the headless
        // loader smokes set the same flag explicitly).
        NODE_OPTIONS: [launch.env.NODE_OPTIONS ?? process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
